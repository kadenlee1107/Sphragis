// Sphragis — Cave Linux Threading (clone()/TLS/scheduler) DESIGN + SKELETON
// =============================================================================
//
// PURPOSE
// This module is the foundation for running multi-threaded ELF binaries under
// the Cave Linux runner. The immediate motivator is Chromium, which even in
...
// the single Cave process. We keep it simple:
//
...
pub fn schedule() {
    ...
    let (next_tid, old_ptr, new_ptr) = {
        ...
        let Some(next_tid) = next_tid_opt else {
            ...
            // have a real deadlock — every Cave thread is parked
            ...
            if crate::caves::linux::skip_log::is_enabled() {
                ...
                if forced_n < 32 {
                    ...
                    if let Some((wtid, reason)) = woken {
                        ...
                        crate::caves::linux::skip_log::record(
                            crate::caves::linux::skip_log::SkipKind::FutexDeadlock,
            ...
            if !DEADLOCK_REPORTED.swap(true, Ordering::AcqRel) {
                ...
                dump();
                crate::caves::linux::syscall_history::dump_per_tid_last();
            }
            return;
        };
...
// Implemented in src/caves/linux/threads.s — saves callee-saved regs of
...
pub fn futex_wait_on(uaddr: u64, val: u32) -> i64 {
    ...
    if !crate::caves::linux::futex::is_valid_uaddr(uaddr) {
        return EAGAIN;
    }
