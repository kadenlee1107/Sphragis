// Sphragis — In-Memory Virtual Filesystem for Cave Containers
...
// Support up to 8 concurrent VFS instances (one per active Cave)
const MAX_VFS_INSTANCES: usize = 8;

/// A complete VFS instance for one Cave.
struct VfsInstance {
    nodes: [VfsNode; MAX_NODES],
    cwd: u16,
    ready: bool,
    cave_id: usize, // which Cave owns this instance
}
...
/// Initialize a VFS instance for a specific Cave.
...
fn populate_rootfs() {
    ...
    if let Some(etc) = find_child(0, b"etc") {
        ...
        if let Ok(hostname) = create_node(etc, b"hostname", NodeType::File, 0o100644) {
            write_file_data(hostname, b"caves\n").ok();
        }
