// Sphragis — Linux epoll(7) implementation for Cave processes
//
// Why this exists:
...
//     timeout is treated as "spin up to N iterations" — the Cave
...
//
// Thread-safety:
//   Sphragis is single-threaded per Cave today, but we still use
...
/// Max number of concurrent epoll instances across all Caves.
...
/// this should go through copy_from_user; for now the Cave shares
...
pub fn epoll_pwait(
    ...
    if !crate::caves::linux::uaccess::is_user_range(events as usize, bytes) {
        return EFAULT;
    }
    ...
    loop {
        ...
        if deadline_ticks != 0
            && crate::caves::linux::threads::cntpct_el0() >= deadline_ticks
        {
            return 0;
        }
        crate::caves::linux::threads::park_current(
            crate::caves::linux::threads::BlockReason::EpollWait {
                epfd,
                deadline_ticks,
            },
        );
...
fn drain_ready(slot: usize, events_ptr: *mut EpollEvent, max: usize) -> usize {
    ...
    use crate::caves::linux::{async_fds, fd as fd_mod, pipe_buf};
    for entry in inst.interests.iter_mut() {
...
pub fn mark_ready(fd: i32, new_events: u32) {
    ...
    if any_match {
        crate::caves::linux::threads::wake_all_epoll_waiters();
    }
}
...
/// the Cave status dump and for tests.
