// Sphragis — File Descriptor Table for Cave Linux Processes
...
fn current_table() -> &'static mut [FdEntry; MAX_FDS] {
    let slot = crate::caves::linux::mmu::current_cave_slot();
    unsafe {
...
pub fn clone_fd_table(child_slot: usize) {
    ...
    unsafe {
        ...
        for entry in tables[child_slot].iter() {
            ...
            match entry.kind {
                FdKind::Eventfd(s) => {
                    crate::caves::linux::async_fds::ref_eventfd_slot(s as usize);
                }
                FdKind::Timerfd(s) => {
                    crate::caves::linux::async_fds::ref_timerfd_slot(s as usize);
                }
                _ => {}
            }
        }
    ...
    unsafe {
        ...
        for i in 0..MAX_FDS {
            ...
            match tables[slot][i].kind {
                FdKind::Eventfd(s) => {
                    let _ = crate::caves::linux::async_fds::free_eventfd_slot(s as usize);
                }
                FdKind::Timerfd(s) => {
                    let _ = crate::caves::linux::async_fds::free_timerfd_slot(s as usize);
                }
                _ => {}
            }
...
pub fn close(fd: u32) -> Result<(), i64> {
    ...
    match table[fd_u].kind {
        FdKind::Eventfd(slot) => {
            let _ = crate::caves::linux::async_fds::free_eventfd_slot(slot as usize);
        }
        FdKind::Timerfd(slot) => {
            let _ = crate::caves::linux::async_fds::free_timerfd_slot(slot as usize);
        }
        _ => {}
    }
