// Sphragis — eventfd + timerfd for Cave Linux
//
...
//   * Reads are non-blocking-or-busy-wait. Since Cave is single-threaded
...
pub fn eventfd2(initval: u32, flags: i32) -> i64 {
    ...
    match crate::caves::linux::fd::alloc_fd_eventfd(slot as u16, fd_flags) {
        Ok(fd) => fd as i64,
        Err(e) => {
            let _ = free_eventfd_slot(slot);
            e
...
pub fn eventfd_read_slot(slot: usize, out_value: *mut u64) -> i64 {
    ...
    if !crate::caves::linux::uaccess::is_user_range(out_value as usize, 8) {
        return EFAULT;
    }
...
pub fn timerfd_create(clockid: i32, flags: i32) -> i64 {
    ...
    match crate::caves::linux::fd::alloc_fd_timerfd(slot as u16, fd_flags) {
        Ok(fd) => fd as i64,
        Err(e) => {
            let _ = free_timerfd_slot(slot);
            e
...
pub fn timerfd_settime(
    ...
    let slot_idx = match crate::caves::linux::fd::timerfd_slot(fd as u32) {
        Some(s) => s,
        None => fd as usize,
    };
    ...
    if !crate::caves::linux::uaccess::is_user_range(new_value as usize, itspec_size) {
        return EFAULT;
    }
    if !old_value.is_null()
        && !crate::caves::linux::uaccess::is_user_range(old_value as usize, itspec_size)
    {
        return EFAULT;
    }
