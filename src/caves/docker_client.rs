// Sphragis — Docker-Cave client (Phase 2 of the design-alignment plan).
//
// ROLE
// ----
...
//   * "Called 'Caves' — containers, sandboxes" (decision log #10)
...
/// `caves create --docker` equivalent. Creates a running container
...
/// `caves run --docker` — exec `argv` inside the cave. Streams stdout
...
/// `caves destroy --docker` — stops + removes the container. Daemon
...
/// `caves list` — returns vector of (name, image, status) for every
