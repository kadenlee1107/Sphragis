...
//
// Example flows:
//   nmap scan → BatPipe → metasploit resource script
//   nmap scan → BatPipe → sqlmap targets
//   nikto scan → BatPipe → burp sitemap
//   gobuster → BatPipe → sqlmap URLs
//   enum4linux → BatPipe → hydra target list
//
...
pub fn export_metasploit(buf: &mut [u8]) -> usize {
    let mut pos = 0;
    let header = b"# BatPipe -> Metasploit Resource Script\n";
