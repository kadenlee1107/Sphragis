...
/// Draw a character at pixel position (x, y) with given foreground/background colors.
pub fn draw_char(fb: *mut u32, screen_w: u32, x: u32, y: u32, ch: u8, fg: u32, bg: u32) {
    let idx = if ch >= 32 && ch <= 126 {
        (ch - 32) as usize
    } else {
...
pub fn draw_str(fb: *mut u32, screen_w: u32, x: u32, y: u32, s: &str, fg: u32, bg: u32) {
    ...
    for byte in s.bytes() {
        if cx + CHAR_W <= screen_w {
            draw_char(fb, screen_w, cx, y, byte, fg, bg);
            cx += CHAR_W;
        }
    }
}
