#!/usr/bin/env python3
"""Headless smoke for `biba-selftest` — gov-grade §3.2 Biba dual lattice."""
...
LOG = (
    ROOT
    / f"logs/qemu-tests/biba-selftest-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
)
...
def main() -> int:
    if not KERNEL.exists():
        print(f"[biba] kernel not found: {KERNEL}", file=sys.stderr); return 2
    print(f"[biba] booting kernel ({KERNEL.stat().st_size:,} bytes)...")
    fp = open(LOG, "wb")
