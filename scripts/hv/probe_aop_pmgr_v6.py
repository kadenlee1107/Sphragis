#!/usr/bin/env python3
"""v6: INBOX without doorbell (journal next-step #1).

v4 showed: INBOX + doorbell → FIQ taken but handler stalls.
v5 showed: RUN alone, no INBOX, no doorbell → FW stays IDLE.
...
def log(m): print(f"[v6] {m}", flush=True)
...
def main():
    ...
    # Write INBOX with SetIOPPower — NO DOORBELL
    log("\n=== INBOX SetIOPPower (NO DOORBELL) ===")
    msg0 = (MGMT_SET_IOP_POWER << 52) | 0x220
    msg1 = 0
    send_inbox(p, msg0, msg1)
    snap(p, "post-INBOX")

    log("polling OUTBOX 15s (no doorbell ring)...")
    msgs = []
    ...
    log(f"\nmsgs: {len(msgs)}")
    if msgs:
        log("*** FW drained INBOX on its own — no doorbell needed! ***")
    else:
        log("FW silent — doorbell likely required; v4 stall is unrelated to the ring itself")
