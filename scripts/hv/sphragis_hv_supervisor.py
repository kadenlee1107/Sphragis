#!/usr/bin/env python3
"""Keep Sphragis running on M4 under m1n1 HV **across resets**.
...
this supervisor hides the reset behind automatic recovery:
  ...
  3. Run sphragis_hv_interactive.py with the user's chosen stimulus,
...
back whenever — Sphragis has been booting, running, and rebooting
...
Env vars (all optional, pass-through to the interactive session):
  SPHRAGIS_KEEP_FB        "1"   — hold the FB live under HV
  SPHRAGIS_HV_STIMULUS    str   — canned ;; -separated shell input to
                               replay on every cycle. Default: the
                               passphrase then a slow uptime poll.
  SPHRAGIS_HV_TIMEOUT     int   — per-cycle hard timeout in seconds
                               (default 360).
  SPHRAGIS_HV_MAX_CYCLES  int   — stop after N cycles (default: run
                               forever).
  SPHRAGIS_HV_LOG_DIR     str   — where to tee per-cycle logs
                               (default /tmp/sphragis_hv_supervisor).
  SPHRAGIS_PASSPHRASE     str   — rebuild-time passphrase info only,
                               not used by the supervisor itself.

Usage:
  sg dialout -c "python3 scripts/hv/sphragis_hv_supervisor.py"
...
INTERACTIVE = ROOT / "scripts/hv/sphragis_hv_interactive.py"
...
def _run_session(cycle: int, log_dir: pathlib.Path) -> tuple[int, int]:
    ...
    """
    stim = os.environ.get("SPHRAGIS_HV_STIMULUS", DEFAULT_STIMULUS)
    keep_fb = os.environ.get("SPHRAGIS_KEEP_FB", "1")
    timeout = int(os.environ.get("SPHRAGIS_HV_TIMEOUT", DEFAULT_TIMEOUT))

    log_path = log_dir / f"cycle_{cycle:04d}.log"
    _log(f"cycle {cycle}: starting HV session → {log_path.name}")

    env = dict(os.environ)
    env["SPHRAGIS_KEEP_FB"] = keep_fb
    env["SPHRAGIS_HV_STIMULUS"] = stim
...
def main():
    log_dir = pathlib.Path(os.environ.get(
        "SPHRAGIS_HV_LOG_DIR", "/tmp/sphragis_hv_supervisor"))
    log_dir.mkdir(parents=True, exist_ok=True)

    max_cycles = int(os.environ.get("SPHRAGIS_HV_MAX_CYCLES", "0"))
