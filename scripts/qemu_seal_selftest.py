#!/usr/bin/env python3
"""Drive cave-seal-selftest in QEMU."""
import pexpect, re, socket, subprocess, sys, time
from pathlib import Path
from datetime import datetime
ROOT = Path(__file__).resolve().parent.parent
KERNEL = ROOT / "target/aarch64-unknown-none/release/bat_os"
LOG = ROOT / f"logs/qemu-tests/seal-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        c.sendline(b"cave-seal-selftest")
        ...
        idx=raw.find("CAVE-SEAL SELF-TEST")
        if idx>=0:
            chunk=raw[idx:]
            end=chunk.find("bat_os >",40)
            print(chunk[:end if end>0 else 1200])
    except pexpect.TIMEOUT: print("[seal] TIMEOUT")
    finally:
