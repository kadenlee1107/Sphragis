...
#
# Inputs:
#   /mnt/d/ai/training/output_v2/final/         — adapter dir from train_lora.py
#   Qwen/Qwen2.5-Coder-7B-Instruct (HF cache) — base model
# Outputs:
#   /mnt/d/ai/training/merged_v2/               — bf16 merged HF model
#   /mnt/d/ai/training/gguf/sphragis-coder-v2.Q4_K_M.gguf
#
# Run:
#   bash scripts/merge_lora_to_gguf.sh
set -euo pipefail

ADAPTER=/mnt/d/ai/training/output_v2/final
MERGED=/mnt/d/ai/training/merged_v2
GGUF_DIR=/mnt/d/ai/training/gguf
GGUF_OUT="$GGUF_DIR/sphragis-coder-v2.Q4_K_M.gguf"
...
ADAPT = "/mnt/d/ai/training/output_v2/final"
OUT   = "/mnt/d/ai/training/merged_v2"
...
"$HOME/ai-venv/bin/python" "$LLAMA_CPP/convert_hf_to_gguf.py" "$MERGED" \
    --outfile "$GGUF_DIR/sphragis-coder-v2.f16.gguf" \
    --outtype f16
...
"$LLAMA_CPP/build/bin/llama-quantize" \
    "$GGUF_DIR/sphragis-coder-v2.f16.gguf" \
    "$GGUF_OUT" \
    Q4_K_M
