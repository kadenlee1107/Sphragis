...
LOG = ROOT / f"logs/qemu-tests/rate-self-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        c.sendline(b"cpol-rate-selftest")
        ...
        idx=raw.find("CPOL-RATE SELF-TEST")
        if idx>=0:
            chunk=raw[idx:]
            end=chunk.find("sphragis >",40)
            print(chunk[:end if end>0 else 1000])
    except pexpect.TIMEOUT: print("[rate-self] TIMEOUT")
    finally:
