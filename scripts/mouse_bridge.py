#!/usr/bin/env python3
"""QMP mouse-injection bridge for Sphragis on macOS.

Restored 2026-05-15 (originally STUMP #110, deleted with the no-
...
- QEMU 10.x on Ubuntu/GTK (the Linux dev path inherits the bridge
  but doesn't need it — GTK delivers virtio events directly)

If you're on macOS with QEMU 10.x and want real mouse pointing,
either downgrade QEMU (`brew install qemu@8`) or switch to the
Ubuntu dev VM where GTK delivers events natively.
