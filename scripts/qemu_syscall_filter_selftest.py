#!/usr/bin/env python3
"""Drive cave-syscall-selftest inside QEMU."""
import pexpect
import socket
import subprocess
import sys
...
LOG = ROOT / f"logs/qemu-tests/sysfilt-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        c.sendline(b"cave-syscall-selftest")
        ...
        idx=raw.find("CAVE-SYSCALL-FILTER SELF-TEST")
        if idx>=0:
            chunk=raw[idx:]
            end=chunk.find("sphragis >",40)
            print(chunk[:end if end>0 else 1200])
    except pexpect.TIMEOUT: print("[sysfilt] TIMEOUT")
    finally:
