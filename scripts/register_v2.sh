#!/usr/bin/env bash
# Register the v2 quantized GGUF with ollama as sphragis-coder-v2.
set -euo pipefail

GGUF=/mnt/d/ai/training/gguf/sphragis-coder-v2.Q4_K_M.gguf
TAG="${TAG:-sphragis-coder-v2}"
MODELFILE=/mnt/d/ai/training/Modelfile.v2

if [ ! -f "$GGUF" ]; then
    echo "[register-v2] FATAL: $GGUF not found"
    exit 1
fi
...
echo "[register-v2] creating tag $TAG"
ollama create "$TAG" -f "$MODELFILE"
echo "[register-v2] available tags:"
