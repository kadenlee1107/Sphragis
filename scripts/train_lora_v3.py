...
def main():
    print(f"[v3-train] base   {BASE_MODEL}", flush=True)
    print(f"[v3-train] data   {DATASET}", flush=True)
    print(f"[v3-train] out    {OUT_DIR}", flush=True)
    print(f"[v3-train] rank={LORA_RANK} alpha={LORA_ALPHA} max_len={MAX_LEN}", flush=True)
    ...
    ds = load_dataset()
    print(f"[v3-train] {len(ds)} records", flush=True)
    ...
    print(f"[v3-train] trainable: {n_train:,} / {n_total:,} ({100*n_train/n_total:.2f}%)", flush=True)
    ...
    print(f"[v3-train] saved -> {final}", flush=True)
