#!/usr/bin/env python3
"""Headless smoke for `te-obj-selftest` — SELinux-style TE on objects."""
...
def main() -> int:
    if not KERNEL.exists():
        print(f"[te-obj] kernel not found: {KERNEL}", file=sys.stderr); return 2
    print(f"[te-obj] booting kernel ({KERNEL.stat().st_size:,} bytes)...")
    fp = open(LOG, "wb")
    ...
    try:
        ...
        print("[te-obj] FAIL — timeout", file=sys.stderr)
        print(f"[te-obj] log: {LOG}", file=sys.stderr); return 1
    finally:
