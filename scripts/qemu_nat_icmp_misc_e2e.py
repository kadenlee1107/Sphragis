...
Three sub-cases driven against the same Bat_OS session:
  ...
  A. Parameter Problem (type 12) from an upstream router — rewritten
     ...
     deferred-2. Proves the `icmp-error-deliv` path now includes the
     new type.

  B. Redirect (type 5) — actively dropped; never reaches the host
     IP stack, never reaches the cave. `icmp-redir-drop` increments.
...
def build_icmp_with_embedded(smac, dmac, router_ip, nic0_ip,
                             err_type, err_code,
                             orig_src_ip, orig_dst_ip, orig_sport, orig_dport,
                             extra_word=0):
    """Outer IPv4 + ICMP error carrying (inner IPv4 + first 8 B L4)."""
    inner_ip = bytearray(20)
    inner_ip[0]=0x45; inner_ip[8]=64; inner_ip[9]=6
    inner_ip[2:4]=(20+20).to_bytes(2,"big")
    ...
    icmp = bytearray(8)
    icmp[0] = err_type; icmp[1] = err_code
    icmp[4:8] = extra_word.to_bytes(4,"big")  # Param Problem uses pointer here
...
def build_icmp_minimal(smac, dmac, router_ip, nic0_ip, icmp_type, payload=b""):
    """Build an outer IPv4 + ICMP with minimal 8 B header + raw payload.
    Used for Redirect (5) and Source Quench (4) where the precise
    inner-header layout doesn't matter for our drop test."""
    icmp = bytearray(8)
    icmp[0] = icmp_type; icmp[1] = 0
...
def parse_outer(f):
    ihl=(f[14]&0x0F)*4
    return {"dst":int.from_bytes(f[14+16:14+20],"big"),
            "proto":f[14+9], "ihl":ihl,
            "type":f[14+ihl] if len(f)>14+ihl else -1}

def main():
    HOST=25578; CAVE=25579
    ...
    try:
        ...
        # Establish NAT entry.
        send_frame(v["conn"], build_tcp_syn(kali_mac,nic1_mac,cave_ip,dst_ip,51234,443))
        ...
        pp = build_icmp_with_embedded(
            gw_mac, nic0_mac, router_ip, nic0_ip,
            12, 0,  # code 0 = "pointer indicates problem"
            nic0_ip, dst_ip, eph, 443,
            extra_word=(12 << 24),  # pointer = 12 (start of src IP field)
        )
        send_frame(h["conn"], pp)
        ...
        # B. Redirect (type 5) — MUST be dropped silently.
        red = build_icmp_minimal(gw_mac, nic0_mac, router_ip, nic0_ip,
                                  5, payload=b"\x00" * 40)
        send_frame(h["conn"], red)
        time.sleep(0.3)
        # Should NOT arrive on cave side.
        redirect_leak = recv_ipv4(v["conn"], 0.8)
        ok_red = redirect_leak is None
        details.append(f"Redirect: {'dropped silently OK' if ok_red else 'LEAKED to cave'}")

        # C. Source Quench (type 4) — MUST be dropped silently.
        sq = build_icmp_minimal(gw_mac, nic0_mac, router_ip, nic0_ip,
                                 4, payload=b"\x00" * 40)
        send_frame(h["conn"], sq)
        time.sleep(0.3)
        sq_leak = recv_ipv4(v["conn"], 0.8)
        ok_sq = sq_leak is None
        details.append(f"SourceQuench: {'dropped silently OK' if ok_sq else 'LEAKED'}")

        stats=run_cmd(c,"nat-stats")
        details.append(stats.strip())
        counters = {"icmp-error-deliv":0, "icmp-redir-drop":0, "icmp-squench-drp":0}
        for line in stats.splitlines():
            for k in counters:
                if k in line:
                    nums=[p for p in line.split() if p.isdigit()]
                    if nums: counters[k] = int(nums[-1])
        ok_counters = (counters["icmp-error-deliv"] >= 1
                       and counters["icmp-redir-drop"] >= 1
                       and counters["icmp-squench-drp"] >= 1)
        details.append(f"counters: {counters}  {'OK' if ok_counters else 'MISMATCH'}")

        if ok_pp and ok_red and ok_sq and ok_counters: verdict = "PASS"
    ...
    print("--- details ---")
    for d in details:
        for line in d.splitlines(): print("  "+line[:220])
    print(f"\nLog: {LOG}")
