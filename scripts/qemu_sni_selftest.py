...
LOG = ROOT / f"logs/qemu-tests/sni-self-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        c.sendline(b"cpol-sni-selftest")
        ...
        idx=raw.find("CPOL-SNI SELF-TEST")
        if idx>=0:
            chunk=raw[idx:]
            end=chunk.find("bat_os >",40)
            print(chunk[:end if end>0 else 1200])
    except pexpect.TIMEOUT: print("[sni-self] TIMEOUT")
    finally:
