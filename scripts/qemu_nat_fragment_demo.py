...
LOG = ROOT / f"logs/qemu-tests/frag-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        c.sendline(b"nat-frag-selftest")
        ...
        idx = raw.find("NAT FRAGMENT CLASSIFIER SELF-TEST")
        if idx >= 0:
            chunk = raw[idx:]
            end = chunk.find("sphragis >", 40)
            print(chunk[: end if end > 0 else 1200])
    except pexpect.TIMEOUT: print("[frag] TIMEOUT")
    finally:
