...
# BAT_OS/M4: P-cluster RVBAR writes SError on t8132 (M4), killing the
# serial link before push_adt. Skip the secondary-CPU RVBAR loop.
parser.add_argument('-S', '--skip-secondary-cpus', action="store_true",
                    help="Skip secondary CPU RVBAR writes (required on M4 t8132)")
...
if args.skip_secondary_cpus:
    print("Skipping secondary CPU RVBAR writes (--skip-secondary-cpus / M4 workaround)")
else:
