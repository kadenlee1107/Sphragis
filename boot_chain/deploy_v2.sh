...
if [ ! -f ./sphragis_apple.macho ]; then
    echo "[!] sphragis_apple.macho not found"
    exit 1
fi
...
# ─── Step 2: Clean up old Sphragis volume if exists ───
echo ""
echo "[Step 2] Preparing volume..."

if diskutil list 2>/dev/null | grep -q "Sphragis"; then
    echo "[*] Removing old Sphragis volume..."
    diskutil apfs deleteVolume "Sphragis" 2>/dev/null || true
    sleep 2
fi

# ─── Step 3: Create volume group (System + Data pair) ───
echo ""
echo "[Step 3] Creating Sphragis volume group..."

# Create the system volume
diskutil apfs addVolume "$CONTAINER" APFS "Sphragis" -role S 2>/dev/null
if [ $? -ne 0 ]; then
    echo "[*] Trying without role flag..."
    diskutil apfs addVolume "$CONTAINER" APFS "Sphragis"
fi
sleep 2
...
if [ ! -d "/Volumes/Sphragis" ]; then
    echo "[!] Failed to mount Sphragis volume"
    echo "    Checking available volumes..."
    ls /Volumes/
    exit 1
fi
echo "[+] Volume mounted at /Volumes/Sphragis"
...
if command -v python3 &>/dev/null; then
    python3 ./make_img4.py ./sphragis_apple.macho ./kernelcache.img4
    IMG4_MADE=$?
else
    echo "[*] python3 not available — using Mach-O directly"
    cp ./sphragis_apple.macho ./kernelcache.img4
    IMG4_MADE=1
fi
...
mkdir -p "/Volumes/Sphragis/System/Library/KernelCollections"
mkdir -p "/Volumes/Sphragis/usr/standalone/firmware"

# Copy to every possible location
cp ./sphragis_apple.macho "/Volumes/Sphragis/System/Library/Kernels/kernel"
echo "  [+] Installed kernel"

if [ -f ./kernelcache.img4 ]; then
    cp ./kernelcache.img4 "/Volumes/Sphragis/System/Library/Caches/com.apple.kernelcaches/kernelcache"
    cp ./kernelcache.img4 "/Volumes/Sphragis/System/Library/KernelCollections/BootKernelExtensions.kc"
    ...
    cp ./sphragis_apple.macho "/Volumes/Sphragis/System/Library/KernelCollections/BootKernelExtensions.kc"
...
# Get the Sphragis volume UUID
VOLUME_UUID=$(diskutil info "Sphragis" 2>/dev/null | grep "Volume UUID" | awk '{print $NF}')
VOLUME_GROUP=$(diskutil info "Sphragis" 2>/dev/null | grep "Volume Group" | awk '{print $NF}' | head -1)
DISK_ID=$(diskutil info "Sphragis" 2>/dev/null | grep "Device Identifier" | awk '{print $NF}')
...
kmutil configure-boot \
    -c "/Volumes/Sphragis/System/Library/Caches/com.apple.kernelcaches/kernelcache" \
    -v "/Volumes/Sphragis" \
...
bless --folder "/Volumes/Sphragis/System/Library/Kernels" \
...
ls -la "/Volumes/Sphragis/System/Library/Kernels/" 2>/dev/null
ls -la "/Volumes/Sphragis/System/Library/Caches/com.apple.kernelcaches/" 2>/dev/null
...
echo ""
echo "  If Sphragis doesn't appear in Startup Manager:"
