...
inline bool validate_utf16(const char16_t* buf, size_t len) { (void)buf; (void)len; return true; }
...
inline size_t utf8_length_from_latin1(const char* in, size_t len) { (void)in; return len * 2; }
inline size_t convert_latin1_to_utf8(const char* in, size_t len, char* out) { (void)out; for(size_t i=0;i<len;i++) out[i]=in[i]; return len; }
inline size_t latin1_length_from_utf8(const char* in, size_t len) { (void)in; return len; }
inline size_t convert_utf8_to_latin1(const char* in, size_t len, char* out) { for(size_t i=0;i<len;i++) out[i]=in[i]; return len; }
inline size_t convert_utf8_to_utf32(const char* in, size_t len, char32_t* out) { (void)in;(void)len;(void)out; return 0; }
inline size_t convert_utf16le_to_latin1(const char16_t* in, size_t len, char* out) { (void)in;(void)len;(void)out; return 0; }
inline size_t convert_latin1_to_utf16le(const char* in, size_t len, char16_t* out) { (void)in;(void)len;(void)out; return 0; }
}
#endif
