...
TF_BUILTIN(GetTemplateObject, CodeStubAssembler) {
  ...
  if (block10.is_used()) {
    ...
    tmp3 = Cast_JSArray_2(state_, TNode<Context>{parameter0}, TNode<Union<HeapObject, Smi, Weak<HeapObject>>>{tmp2}, &label4);
    ca_.Goto(&block12);
    if (label4.is_used()) {
      ca_.Bind(&label4);
      ca_.Goto(&block13);
  ...
  if (block16.is_used()) {
    ca_.Bind(&block16);
    CodeStubAssembler(state_).StoreFeedbackVectorSlot(TNode<FeedbackVector>{tmp9}, TNode<UintPtrT>{parameter3}, TNode<Union<HeapObject, Smi, Weak<HeapObject>>>{tmp8});
    CodeStubAssembler(state_).Return(tmp8);
  }
}
...
TF_BUILTIN(ForInNext, CodeStubAssembler) {
  ...
  if (block5.is_used()) {
    ...
    tmp15 = CodeStubAssembler(state_).TaggedEqual(TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp14}, TNode<Object>{parameter4});
...
// https://crsrc.org/c/v8/src/builtins/internal.tq?l=33&c=20
