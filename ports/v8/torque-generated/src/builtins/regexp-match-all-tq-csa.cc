...
TNode<JSAny> RegExpPrototypeMatchAllImpl_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<NativeContext> p_nativeContext, TNode<JSAny> p_receiver, TNode<JSAny> p_string) {
  ...
  if (block4.is_used()) {
    ...
    tmp31 = UnsafeCast_FastJSRegExp_0(state_, TNode<Context>{p_context}, TNode<Object>{tmp0});
    tmp32 = RegExpBuiltinsAssembler(state_).FastLoadLastIndex(TNode<JSRegExp>{tmp31});
    RegExpBuiltinsAssembler(state_).FastStoreLastIndex(TNode<JSRegExp>{tmp30}, TNode<Smi>{tmp32});
    tmp33 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp>{tmp30}, CastIfEnumClass<JSRegExp::Flag>(JSRegExp::Flag::kGlobal));
    tmp34 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp>{tmp30}, CastIfEnumClass<JSRegExp::Flag>(JSRegExp::Flag::kUnicode));
    ca_.Branch(tmp34, &block10, std::vector<compiler::Node*>{}, &block11, std::vector<compiler::Node*>{});
  }
  ...
  if (block10.is_used()) {
    ...
    ca_.Goto(&block12, tmp35);
  }

  TNode<BoolT> tmp36;
  if (block11.is_used()) {
    ca_.Bind(&block11);
    tmp36 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp>{tmp30}, CastIfEnumClass<JSRegExp::Flag>(JSRegExp::Flag::kUnicodeSets));
  ...
  if (block12.is_used()) {
    ...
    ca_.Goto(&block2, tmp29, tmp33, phi_bb12_16);
  }
  ...
  if (block14.is_used()) {
    ca_.Bind(&block14);
    tmp38 = FromConstexpr_Smi_constexpr_IntegerLiteral_0(state_, IntegerLiteral(true, 0x1ull));
    tmp39 = CodeStubAssembler(state_).SmiNotEqual(TNode<Smi>{tmp24}, TNode<Smi>{tmp38});
    ca_.Goto(&block15, tmp39);
  }
  ...
  if (block2.is_used()) {
    ...
    tmp40 = RegExpMatchAllAssembler(state_).CreateRegExpStringIterator(TNode<NativeContext>{p_nativeContext}, TNode<JSAny>{phi_bb2_6}, TNode<String>{tmp1}, TNode<BoolT>{phi_bb2_7}, TNode<BoolT>{phi_bb2_8});
    ca_.Goto(&block16);
  }

    ca_.Bind(&block16);
  return TNode<JSAny>{tmp40};
}
...
TF_BUILTIN(RegExpStringIteratorPrototypeNext, CodeStubAssembler) {
  ...
  if (block12.is_used()) {
    ...
    tmp19 = Null_0(state_);
    tmp20 = CodeStubAssembler(state_).TaggedEqual(TNode<Object>{tmp18}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp19});
  ...
  if (block31.is_used()) {
    ...
    tmp50 = CodeStubAssembler(state_).TaggedEqual(TNode<Object>{tmp47}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp49});
  ...
  if (block25.is_used()) {
    ...
    tmp61 = CodeStubAssembler(state_).TaggedEqual(TNode<Object>{tmp59}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp60});
