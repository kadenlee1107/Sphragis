...
TF_BUILTIN(NumberPrototypeToString, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    tmp4 = CodeStubAssembler(state_).TaggedEqual(TNode<Object>{tmp2}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp3});
...
TF_BUILTIN(ParseInt, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    tmp1 = CodeStubAssembler(state_).TaggedNotEqual(TNode<Object>{parameter2}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp0});
...
// https://crsrc.org/c/v8/src/builtins/number.tq?l=59&c=10
