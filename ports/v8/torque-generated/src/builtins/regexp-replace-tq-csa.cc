...
TNode<String> RegExpReplaceFastGlobalCallable_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>> p_replaceFn) {
  ...
  if (block0.is_used()) {
    ...
    tmp4 = Null_0(state_);
    tmp5 = CodeStubAssembler(state_).TaggedEqual(TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp2}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WeakCell>>{tmp4});
...
// https://crsrc.org/c/v8/src/builtins/regexp-replace.tq?l=136&c=1
TNode<String> RegExpReplaceFastString_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string, TNode<String> p_replaceString) {
  ...
  if (block13.is_used()) {
    ...
    tmp16 = ca_.CallBuiltin<String>(Builtin::kSubString, p_context, p_string, tmp15, tmp13);
    tmp17 = StringAdd_0(state_, TNode<Context>{p_context}, TNode<String>{tmp0}, TNode<String>{tmp16});
    tmp18 = FromConstexpr_Smi_constexpr_IntegerLiteral_0(state_, IntegerLiteral(false, 0x0ull));
    tmp19 = CodeStubAssembler(state_).SmiNotEqual(TNode<Smi>{tmp2}, TNode<Smi>{tmp18});
    ca_.Branch(tmp19, &block15, std::vector<compiler::Node*>{}, &block16, std::vector<compiler::Node*>{tmp17});
  }

  TNode<String> tmp20;
  if (block15.is_used()) {
    ca_.Bind(&block15);
    tmp20 = StringAdd_0(state_, TNode<Context>{p_context}, TNode<String>{tmp17}, TNode<String>{p_replaceString});
    ca_.Goto(&block16, tmp20);
  }
  ...
  if (block16.is_used()) {
    ...
    tmp21 = CodeStubAssembler(state_).LoadStringLengthAsSmi(TNode<String>{p_string});
    tmp22 = ca_.CallBuiltin<String>(Builtin::kSubString, p_context, p_string, tmp14, tmp21);
    tmp23 = StringAdd_0(state_, TNode<Context>{p_context}, TNode<String>{phi_bb16_4}, TNode<String>{tmp22});
...
// https://crsrc.org/c/v8/src/builtins/regexp-replace.tq?l=48&c=31
...
// https://crsrc.org/c/v8/src/builtins/regexp-replace.tq?l=145&c=22
...
// https://crsrc.org/c/v8/src/builtins/regexp-replace.tq?l=179&c=10
