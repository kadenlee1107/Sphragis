...
TNode<RegExpMatchInfo> RegExpPrototypeExecBodyWithoutResult_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string, TNode<Number> p_regexpLastIndex, bool p_isFastPath, compiler::CodeAssemblerLabel* label_IfDidNotMatch) {
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp1 = CodeStubAssembler(state_).SmiConstant(CastIfEnumClass<int31_t>(v8::Isolate::kRegExpExecCalledOnSlowRegExp));
...
TNode<BoolT> IsRegExp_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSAny> p_obj) {
  ...
  if (block14.is_used()) {
    ca_.Bind(&block14);
    tmp11 = CodeStubAssembler(state_).SmiConstant(CastIfEnumClass<int31_t>(v8::Isolate::kRegExpMatchIsTrueishOnNonJSRegExp));
  ...
  if (block20.is_used()) {
    ca_.Bind(&block20);
    tmp14 = CodeStubAssembler(state_).SmiConstant(CastIfEnumClass<int31_t>(v8::Isolate::kRegExpMatchIsFalseishOnJSRegExp));
