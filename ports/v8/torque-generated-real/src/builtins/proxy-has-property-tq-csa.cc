...
TF_BUILTIN(ProxyHasProperty, CodeStubAssembler) {
  ...
  if (block28.is_used()) {
    ...
    tmp8 = GetMethod_2(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp2}, "has", &label9);
    ca_.Goto(&block32);
    if (label9.is_used()) {
      ca_.Bind(&label9);
      ca_.Goto(&block33);
  ...
  if (block32.is_used()) {
    ca_.Bind(&block32);
    tmp10 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<JSAny>{tmp8}, TNode<JSAny>{tmp2}, TNode<JSAny>{tmp6}, TNode<JSAny>{parameter2});
...
// https://crsrc.org/c/v8/src/builtins/proxy-has-property.tq?l=18&c=10
...
// https://crsrc.org/c/v8/src/builtins/proxy-has-property.tq?l=25&c=37
