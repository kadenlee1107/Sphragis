...
tmp25 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(0)), tmp18, TNode<Object>(), tmp24, phi_bb25_17));
  ...
  if (block14.is_used()) {
    ...
    tmp54 = TypedArraySpeciesCreateByLength_0(state_, TNode<Context>{parameter0}, kBuiltinNameFilter_0(state_), TNode<JSTypedArray>{tmp0}, TNode<UintPtrT>{tmp53}, CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kWrite));
    ...
    tmp58 = CodeStubAssembler(state_).LoadJSArrayElementsMap(CastIfEnumClass<ElementsKind>(ElementsKind::PACKED_ELEMENTS), TNode<NativeContext>{tmp57});
