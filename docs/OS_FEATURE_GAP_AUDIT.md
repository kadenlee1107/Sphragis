...
| 030 | Per-cave CPU/memory/IO quotas | **Partial.** Memory quota enforced at shm + pipe + BatFS (`batfs::ns_create` / `ns_delete` charge & release through `cave::active_charge_pages`); CPU + net are observability counters (no enforcement until preemptive timer scheduling re-enabled — see deferred list). `batfs-quota-selftest` proves the BatFS path. | commits `6715c827` + `18890477` + 2026-05-12 batfs-quota |
...
After this refresh, the still-open P1 list narrows to:
  ...
  * 044 VLAN (802.1Q)
  * 045 conntrack-class stateful firewall — **foundation shipped.** `src/net/conntrack.rs` adds a 64-slot flow table keyed on (proto, remote_ip, remote_port, local_port) with `New` / `Established` / `Closed` states; `tcp::connect_start` registers and `tcp::connect_blocking_pcb` marks established on SYN-ACK; `tcp::close_pcb` releases. `conntrack-selftest` proves the lifecycle. Driving the firewall decision from this table (and removing the wildcard inbound TCP allow) is the next hardening pass.
