...
- ✅ AES-256, ML-KEM crate, ML-DSA crate, audit-chain HMAC, SealFS AES-GCM-SIV, PAN, BTI, per-cave ASIDs — all landed
...
| ISO-004 | P0 | ⚠️ PARTIAL | Week 3-4 closed Cave-H6 (`sys_connect` gates on cave_policy). **`scripts/audit_syscall_policy.py` + `.github-workflows-pending/syscall-policy-lint.yml`** (SP-ISO-004 lint drafted 2026-05-16): scans every `(pub )?fn sys_*` handler in `src/`, classifies PASS-policy (references cave_policy/active_has_cap/has_cap) / PASS-annotated (carries `// cave-policy: not-required (<reason>)` comment) / WARN-no-gate. Baseline today: 91 WARN (mostly Linux-compat ENOSYS stubs in src/caves/linux/syscall.rs — triage to SP-ISO-004.1 annotation pass). Workflow runs report-only initially; flips to `--fail-on-warn` after triage. Workflow YAML OAuth-blocked; user copies to `.github/workflows/`. |
...
| REQ | P | Status | Notes |
|---|---|---|---|
| ATT-001 | P0 | ⚠️ PARTIAL | `src/security/attest.rs` (SP-C1.1 + SP-C1.2) defines API: `Claims`, `Quote`, `KernelMeasurement`, `CaveIdentity`, `quote()`, `verify_quote_local()`. Signature is ML-DSA-87 (CNSA 2.0 cat-5). **SP-C1.2 wired real kernel measurement** — SHA-384 of `__text_start..__text_end + __rodata_start..__rodata_end` computed at boot via `init_kernel_measurement()`, cached in `MEASUREMENT`. **SP-ATT-001 wire format + external verifier landed** (2026-05-16): `Quote::to_wire()` serializes to SPHATTV1 binary format; `attest-dump` shell command writes `attest-quote.bin` to SealFS; `tools/attest-verifier/attest_verifier.py` parses the wire format, structurally validates every length (KERNEL_MEAS_LEN=48, CAVE_MEAS_LEN=48, NONCE_LEN=32, MLDSA87_PK_LEN=2592, MLDSA87_SIG_LEN=4627), re-derives the canonical `signed_payload`, and cryptographically verifies the ML-DSA-87 signature if `pqcrypto-mldsa` or `liboqs-python` is installed (structural-only otherwise). Round-trip + bad-magic tests pass offline. In-memory attestation key still today; hardware-rooted key chain is SP-C1.4 (SEP) / SP-C1.5 (Caliptra) — PARTIAL until that lands. CBOR-per-RATS encoding is SP-C1.7. |
...
| BLD-003 | P0 | ⚠️ PARTIAL | `scripts/build_intoto_attestation.py` exists. **`.github-workflows-pending/intoto-attest.yml`** drafted 2026-05-16: on every v* tag, builds release kernel + runs the in-toto script + uploads `sphragis.intoto.jsonl` + `.sha256` to the GitHub Release. Together with sigstore (BLD-005) + SBOM (BLD-004) forms the SLSA L3 → L4 chain (release-sign attests *who*, in-toto attests *how*, SBOM attests *what*). Workflow YAML OAuth-blocked; user copies to `.github/workflows/`. After first tag publishes an in-toto attestation, BLD-003 promotes PARTIAL → HAVE. |
| BLD-004 | P0 | ⚠️ PARTIAL | `scripts/gen_sbom.py` + `scripts/generate_sbom.py` exist. **`.github-workflows-pending/sbom-publish.yml`** drafted 2026-05-16: on every push to main + every v* tag, regenerates the CycloneDX 1.5 SBOM, validates it parses + has ≥40 components, emits SHA-256, and (on tag) uploads `sbom.cdx.json` + `.sha256` to the GitHub Release. Workflow YAML OAuth-blocked; user copies to `.github/workflows/`. After first tag publishes a signed SBOM, BLD-004 promotes PARTIAL → HAVE. |
...
| UX-004 | P0 | ⚠️ PARTIAL | `DESIGN_PRODUCTIZATION_UX.md` §4 published — per-user identity model (uuid + username + Argon2id passphrase + optional FIDO2 + capabilities); 5 roles (Operator, CryptoOfficer, AuditOfficer, CaveAdmin, User); SealFS storage; 4-step IMPL phasing. |
...
| HW-002 | P0 | ⚠️ PARTIAL | `DESIGN_X86_64_PORT.md` published (SP-HW-002). Target hardware (Intel NUC 13 Pro + Lenovo ThinkPad X1 Carbon Gen 11). 9 architectural decisions (UEFI+LMS boot, TPM 2.0 attestation root, PCID per-cave, SMEP+SMAP+UMIP+CET, Spectre v2 IBRS/STIBP/IBPB, driver layer per subsystem, SealFS on NVMe, CVM compatibility). Sphragis-side scope (which modules change). 6-step IMPL phasing (.A minimum viable boot, .B drivers, .C security primitives, .D TPM attestation, .E physical hardware bring-up, .F confidential VM). Closes REQ-HW-006 QEMU x86_64 CI in .A. |
