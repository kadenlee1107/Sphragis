...
| DOC-004 | P0 | ⚠️ PARTIAL | `docs/templates/CAPABILITY_STATEMENT_TEMPLATE.md` published (SP-DOC-004 template). 8-page gov-procurement-standard format covering exec summary + 5 core capabilities + tech readiness + differentiators vs incumbents + past performance + cert posture + NAICS + small-business status + differentiating clauses + contact + how-to-use instructions. Founder fills in `{PLACEHOLDER}` values + renders to PDF for distribution. |
...
| DOC-006 | P0 | ⚠️ PARTIAL | `docs/NIST_800_53_INHERITANCE.md` v1.1 (SP-DOC-006). **AC family complete (25 controls), AU family complete (16 controls)** plus STARTER coverage of CM/IA/SC/SI/MP/SA/SR/PT (~25 controls). Total ~66 controls. 33 SATISFIED, 18 PARTIAL, 4 HYBRID, 3 CUSTOMER, 8 N/A. SP-DOC-006.FULL extends to remaining families. |
