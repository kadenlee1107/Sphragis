...
- **Repo visibility:** PRIVATE on GitHub (`kadenlee1107/Sphragis`).
...
from now), Sphragis will be released under a **dual license**:

1. **AGPL-3.0** for open-source / research / non-commercial use.
2. **Commercial license** sold separately by Kaden Lee (the copyright
   holder) for any party that wants to use Sphragis in a closed-source
...
| **Discourage freeloading.** A contractor cannot quietly fork Sphragis into a closed product without either open-sourcing their fork OR paying us. | AGPL's "network-use is distribution" clause closes the SaaS loophole that GPL leaves open. |
...
- **Apache-2.0 / MIT (permissive).** Anyone can fork Sphragis into a
...
- **GPL-3.0 (no network clause).** Closes the binary-distribution
  ...
  can host a Sphragis-derived service without ever re-distributing
  the source. AGPL-3.0 covers both.

## Dependency policy

**Sphragis source code may not absorb code under copyleft licenses
(GPL, AGPL, LGPL, MPL, EUPL, etc.).** Linking against a copyleft
crate transitively forces Sphragis to inherit that license, which
...
The name "Sphragis" and any project logo should be claimed as a
...
products. Domain reservation (sphragis.dev / sphragis.dev / etc.) is
