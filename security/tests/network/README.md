# Sphragis network fuzz harness

Scripts here exercise specific ATTACK-NET-* findings in
`../../PENTEST_NETWORK.md`. They assume:

* Sphragis running in QEMU (see repo root `Makefile`'s `run` target).
...
In this mode the **host can reach Sphragis at 10.0.2.15 ONLY through the
hostfwd redirection**. You can send IP/TCP/UDP to `127.0.0.1:2222`
(which Sphragis sees on port 2222). L2 tricks (ARP poison, malformed
...
Sphragis initiating a TCP connection to a remote host that we control.
...
python3 fuzz_dns.py             # 035, 036, 039, 041 (needs Sphragis to call resolve())
...
Each script prints "SENT <attack-id>" and waits. Watch the Sphragis UART
...
|--------|------------|--------------|
| fuzz_arp.py | 001, 002 | Injects gratuitous ARP replies claiming to be 10.0.2.2. Then, checks by sending a ping from 10.0.2.1; if Sphragis MACs its reply to the poisoned MAC, win. |
| fuzz_ip.py | 004, 005, 008 | Crafts IPv4 frames with IHL<5, IHL>total_len, and wrong checksum. Watch for kernel panic (ATTACK-005). |
| fuzz_tcp.py | 009, 010, 011, 019, 022 | Predicts Sphragis ISN (deterministic), then injects blind RST, blind FIN, blind data. |
...
these scripts should all produce `REJECTED` on the Sphragis UART (firewall
