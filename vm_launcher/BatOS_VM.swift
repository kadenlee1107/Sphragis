// Bat_OS — Native M4 VM Launcher
// Uses Apple's Virtualization.framework to run Bat_OS
// directly on M4 silicon. No emulation — your actual CPU
// cores execute the Bat_OS kernel at native speed.
//
// Usage: swift BatOS_VM.swift <path_to_kernel>
...
if CommandLine.arguments.count > 1 {
    ...
    kernelPath = "\(home)/Bat_OS/target/aarch64-unknown-none/release/bat_os"
}

guard FileManager.default.fileExists(atPath: kernelPath) else {
    print("Error: Kernel not found at \(kernelPath)")
    print("Build it first: cd ~/Bat_OS && cargo build --release")
    exit(1)
}

print("""
══════════════════════════════════════════════
  BAT_OS — Native M4 Virtual Machine
══════════════════════════════════════════════
  Kernel: \(kernelPath)
...
let logPath = NSTemporaryDirectory() + "bat_os_serial.log"
