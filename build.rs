...
//
// Consumed by src/main.rs via option_env!("SPHRAGIS_PASSPHRASE") /
// option_env!("SPHRAGIS_DURESS"). Leave unset in production — main.rs falls
// through to the interactive UART prompt + kernel-image-hash derivation
// for duress (see DEV_FALLBACK_LABEL / DURESS_LABEL).
fn main() {
    println!("cargo:rerun-if-env-changed=SPHRAGIS_PASSPHRASE");
    println!("cargo:rerun-if-env-changed=SPHRAGIS_DURESS");
    ...
    println!("cargo:rerun-if-env-changed=SPHRAGIS_DISABLE_INIT_TRAMPOLINE");
    ...
    println!("cargo:rerun-if-env-changed=SPHRAGIS_RELEASE_PUBKEY");
    ...
    // because the new stack never made it into sphragis.bin.
    ...
    // monotonic floor without a wall-clock RTC. Sphragis is bare-metal
    ...
    println!("cargo:rustc-env=SPHRAGIS_BUILD_UNIX={build_unix}");
